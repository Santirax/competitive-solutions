...
int main(){
  ...
  while (T--){
    ...
    for(int i = 0; i < N; i++){
      cin>>comand;
      cin>>dist;
      if(comand == "fd"){//Mover adelante

      } else if(comand == "rt"){//girar a la derecha dist grados

      } else if(comand == "lt"){//girar a la izquierda dist grados

      } else if(comand == "bk"){//Mover atras
  ...
  while(T--){
    ...
    for(int i = 0; i < N; i++){
      ...
      if(cmd == "lt"){
        ...
        x = x*cos(((arg*PI)/180.0)) - y*sin(((arg*PI)/180.0));
        y = x*sin(((arg*PI)/180.0)) + y*cos(((arg*PI)/180.0));
