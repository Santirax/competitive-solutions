...
int main (){
  ...
  cin>>T;
  while (T--){
    //Intervalos de las cortinas
    vector<Double>intervalos;
    //Guardamos la linea (cortina) y su transparencia
    vector<pair<Linea, double> > cortinas;
    //el par ya acostumbrado auxiliar
    pair<Linea, double> l_conTrans;

    //Linea del eje de las "y" para poder proyectar las paralelas en los intervalos
    Linea paralell = Linea(Punto(0,0), Punto (0, INF));

    int N;
    cin>>N;
    //coordenadas y la tranparencia de cada cortina
    double x1, y1, x2, y2, trans;
    Linea l;
    //El primer intervalo tiene que ser desde -infinito
    intervalos.push_back(-INF);
    //Leemos datos y guardamos
    for(int x = 0; x < N; x++){
      cin>>x1>>y1>>x2>>y2>>trans;

      l = Linea(Punto(x1, y1), Punto(x2, y2));
      l_conTrans.first = l;
      l_conTrans.second = trans;
      cortinas.push_back(l_conTrans);

      intervalos.push_back(x1);
      intervalos.push_back(x2);
    }
    //Y el ultimo intervalo a infinito
    intervalos.push_back(INF);

    //Declaramos los dos puntos de prueba en los cuales vamos a proyectar las rectas
    //paralelas al eje y
    Punto PuntoPrueba1, PuntoPrueba2;

    //Linea Punto de Prueba LPP;
    //Esta ya es la recta que va a pasar por los intervalos
    Linea LPP1, LPP2;
    cout<<fixed<<setprecision(3);
    //Algo impostante es ordenar los intervalor porque no te los dan en orden
    sort(intervalos.begin(), intervalos.end());

    /*for(int i = 0; i < cortinas.size(); i++){
      cout<<cortinas[i].second<<endl;
    }*/
    //Imprimir cuantos intervalos son
    cout<<intervalos.size()-1<<endl;

    /*Esto se hace por cada intervalo*/
    for(int i = 0; i < intervalos.size()-1; i++){
      //Extremo izquierdo del intervalo, el +0.001 es porque el intervalo es abierto
      PuntoPrueba1 = Punto(intervalos[i]+0.001, 0);
      //Extremo izquierdo del intervalo, el -0.001 es porque el intervalo es abierto
      PuntoPrueba2 = Punto(intervalos[i+1]-0.001, 0);
      //Lineas de prueba por cada intervalo, tanto extremo izquierdo como derecho, intervalo abierto
      LPP1 = ParalelaEnPunto(paralell, PuntoPrueba1);//izquierdo
      LPP2 = ParalelaEnPunto(paralell, PuntoPrueba2);//derecho
      //Nuestro res que es el valor total de transparencia
      double res = 1;
      //Por cada cortina vemos si intersecta con las lineas propuestas, si intersecta
      //Es que esa cortina vive en el intervalo y debemos considerar su valor de transparencia
      for(int j = 0; j < cortinas.size(); j++){
        int x1 = IntersecRectaSegmen(LPP1, cortinas[j].first);
        int x2 = IntersecRectaSegmen(LPP2, cortinas[j].first);
        if(x1 != 0 || x2 != 0){
          //Aumentamos el res con el valor de transparencia de la cortina
          res*=cortinas[j].second;
        }
      }

      //Esto es mera impresion con el formato pedido por el problema
      if(intervalos[i] == -INF)
       cout<<"-inf ";
        else
         cout<<intervalos[i]<<" ";

      if(intervalos[i+1] == INF)
       cout<<"+inf ";
        else
         cout<<intervalos[i+1]<<" ";

      if(intervalos[i] == -INF || intervalos[i+1] == INF)
        cout<<"1.000\n";
         else
            cout<<res<<"\n";
      //cout<<intervalos[i]<<" "<<intervalos[i+1]<<endl;
      //cout<<intervalos[i]<<" ";
    }
