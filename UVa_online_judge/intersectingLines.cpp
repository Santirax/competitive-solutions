...
int main(){
  ...
  int N;
  cin>>N;
  /*Todo es sacado del reference que paso Manzano*/
  cout<<"INTERSECTING LINES OUTPUT\n";
  while (N--){
    int x;
    /*Escaneamos las coordenadas de las lineas*/
    vector<int> coordenadas;
    for(int i = 0; i < 8; i++){
      cin>>x;
      coordenadas.push_back(x);
    }
    /*Declaramos y rellenamos las dos lineas*/
    Linea l1, l2;
    l1 = Linea( Punto(coordenadas[0], coordenadas[1]), Punto( coordenadas[2], coordenadas[3] ) );
    l2 = Linea( Punto(coordenadas[4], coordenadas[5]), Punto( coordenadas[6], coordenadas[7] ) );
    int ban = 0;
    /*Despues mandamos a llamar la funcion InterseccionRectas, para saber si se
    intersectan en un punto (1), infinitos puntos (-1) o no se intersectan (0)*/
    ban = InterseccionRectas(l1, l2);
    if(ban == 0)
     cout<<"NONE\n";
      else if(ban == -1)
       cout<<"LINE\n";
        else {
          /*En caso de que se intersecten mandar a llamar la funcion que
          me dice en que punto se intersecto e imprimir el resultado*/
           Punto interseccion;
           interseccion = PuntoInterseccion(l1, l2);
           cout<<fixed<<setprecision(2);
           cout<<"POINT "<<interseccion.x<<" "<<interseccion.y<<"\n";
        }
