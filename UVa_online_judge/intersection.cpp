...
int main(){
  ...
  while(N--){
    ...
    for(int i = 0; i < 8; i++){
      int q;
      cin>>q;
      coordenadas.push_back(q);
    }

    //Guardamos la linea
    l = Linea( Punto(coordenadas[0], coordenadas[1]), Punto(coordenadas[2], coordenadas[3]) );
     int ban1 = 0, ban2 = 0, banAux = 0, ban3 = 0;

    //Metemos los puntos dados a un poligono
    Poligono rect;
    rect.push_back(Punto(coordenadas[4], coordenadas[5]));
    rect.push_back(Punto(coordenadas[4], coordenadas[7]));
    rect.push_back(Punto(coordenadas[6], coordenadas[5]));
    rect.push_back(Punto(coordenadas[6], coordenadas[7]));
    //El Convex Hull se encarga de hacer todo lo necesario para que
    //quede como un poligono descente
    rect = CercoConvexo(rect);
    //despues vemos si cualquiera de los puntos de la recta viven en el rect o si estan en
    //El perimetro
    ban1 = PuntoEnConvexo(Punto(coordenadas[0], coordenadas[1]), rect);
    ban2 = PuntoEnConvexo(Punto(coordenadas[2], coordenadas[3]), rect);
    if(ban1 != 0){
      ban1 = 1;
    }
    if(ban2 != 0){
    ...
    /*Posteriormente hacemos todas las rectas que tiene el rectangulo
    y vemos si al menos una intersecta con la linea dada*/
    vector<Linea> segmentosRect;
    segmentosRect.push_back(Linea( Punto(coordenadas[4], coordenadas[5]), Punto(coordenadas[6], coordenadas[5]) ));
    segmentosRect.push_back(Linea( Punto(coordenadas[6], coordenadas[5]), Punto(coordenadas[6], coordenadas[7]) ));
    segmentosRect.push_back(Linea( Punto(coordenadas[6], coordenadas[7]), Punto(coordenadas[4], coordenadas[7]) ));
    segmentosRect.push_back(Linea( Punto(coordenadas[4], coordenadas[7]), Punto(coordenadas[4], coordenadas[5]) ));

    /*O si intersecta con un punto del "rectangulo"*/
    for(int i = 0; i < 4; i++){
      banAux = InterseccionSegmentos(l, segmentosRect[i]);
      if(banAux == 1 || banAux == -1)
       ban3 = 1;
    }
    /*Cualquiera de las tres o las tres condiciones son validas*/
