...
int main(){
  ...
  while(cin>>N && N != 0){
    ...
    //cout<<p[0].x<<" and "<<p[0].y<<endl;
    Double areaP = 0, areaCH = 0;
    p.push_back(p[0]);
    areaP = Area(p);
    convexHull = CercoConvexo(p.begin(), p.end());
    //cout<<p.size()<<endl;
    areaCH = Area(convexHull);
    Double regla3 = (areaP*100)/areaCH;

    cout<<"Tile #"<<cont<<endl;
    cout<<fixed<<setprecision(2);
    cout<<"Wasted Space = "<<100.0-regla3<<" %\n\n"; 
